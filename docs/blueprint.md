...
- **Strands Agents SDK**: エージェント実装（`Agent()` + `@tool` + `GraphBuilder`）
  > **実装メモ**: `Swarm` は自律 handoff で発言順・回数が不安定だったため、`GraphBuilder`（deterministic graph）に変更。3ノード固定チェーン: `claude_engineer → gpt_tax_advisor → gemini_mother` + Graph外で `claude_summarizer` を全出力付きで呼び出し。詳細は §16 参照
...
> **実装メモ**: 実際には `claude_summarizer` を Graph 外で独立呼び出しする方式に変更した。`claude_engineer`（レビュアー）と `claude_summarizer`（まとめ役）は同じ Anthropic Claude モデルだが、別の Agent インスタンス・別のシステムプロンプトで動作する。Graph 内の線形チェーンでは直前ノードの出力しか渡されないため、まとめ役には全3エージェントの出力を結合して渡す。
...
GEMINI_MODEL_ID="gemini-3.5-flash"            # Gemini（母ちゃん）
OPEN_AI_MODEL_ID="gpt-5.6-terra"                          # GPT（税理士）
...
**採用した方式**: `strands.multiagent.graph.GraphBuilder` で 3ノードの固定チェーンを構築し、まとめ役は Graph 外で全出力を渡して呼び出す:
```
Graph: claude_engineer → gpt_tax_advisor → gemini_mother
↓ 全3エージェントの出力を結合
claude_summarizer（Graph外で直接呼び出し）
```
- 発言順序が決定的で、全員が確実に1回ずつ発言する
- `claude_summarizer` は Graph 外で全エージェントの出力を受け取って総括する（Graph 内に入れると直前ノードの出力しか見えない fan-in バグがあったため分離）
- `execution_timeout=3000.0`, `node_timeout=1200.0`

### claude_summarizer の分離

`claude_engineer` にレビューとまとめを兼任させると、キャラのトーンが混在して出力品質が不安定だった。まとめ役を `claude_summarizer` として独立させ、専用のシステムプロンプト (`CLAUDE_SUMMARIZER_PROMPT`) を持たせた。GraphBuilder の線形チェーンでは各ノードが直前ノードの出力しか受け取れないため、`claude_summarizer` を Graph 外に出し、全3エージェントの出力を結合して渡す方式に変更した。
...
| ECS Cluster | `okamo-channel` |
| Task Definition | `okamo-channel-daily:9` (1vCPU / 4GB, FARGATE。`CLAUDE_MODEL_ID=claude-sonnet-5`, `OPEN_AI_MODEL_ID=gpt-5.6-terra`) |
