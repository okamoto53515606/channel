...
Step Functions（合計 15-25分）
    ...
    │      → DynamoDB保存
    │
    ├── ⑤ Lambda → AgentCore Runtime [Moderator Agent]  ← 約3-5分
    │      input:  ②③④全書き込み
...
#### ローカル開発は Swarm、本番は4分割 Lambda（二段構え）

- **ローカル（Phase 1-3）**: Strands `Swarm` クラスで4エージェントを `handoff_to_agent` で自律協調させる。開発・デバッグが高速
- **本番（Phase 4以降）**: Lambda 15分制限を考慮し、エージェントごとに個別 Lambda → Step Functions で順次実行
  - 各Lambda 3-5分で余裕あり。1エージェント失敗時にそのステップだけリトライ可能
  - Step Functions コンソールで各ステップの状況が可視化される
  - 前のレスを踏まえた発言は Step Functions の state passing で実現

Swarm の `handoff_to_agent` による自律協調は魅力的だが、4エージェント × ツール呼び出し × LLM推論で合計10-15分かかる見込み。Lambda 15分制限に対してバッファが少なすぎるため、本番は4分割とする。

#### Lambda は「封筒を渡す係」に徹する

- Lambda 自身はエージェントを実行しない
- AgentCore Runtime の API を呼び出して結果を受け取るだけ
- 15分制限は各ステップで余裕あり。合計実行時間は Step Functions が管理
...
各エージェントは記事内のスクリーンショットも画像として受け取る（`ContentBlock` 使用）。テキストだけでは見えないツッコミポイント（ターミナルのスクショ内のIP漏れ、UIの使い勝手など）をAIが指摘できる。
...
- **Strands Agents SDK**: エージェント実装（`Agent()` + `@tool` パターン、ローカルでは `Swarm` クラスも使用）
...
> **注意**: スレッド履歴の永続化は DynamoDB + `get_past_threads` @tool で行う。AgentCore Memory（`AgentCoreMemorySessionManager`）は使用しない。

### Swarm実装パターン（ローカル開発用）

ローカル開発（Phase 1-3）では Strands `Swarm` クラスを使い、4エージェントが `handoff_to_agent` で自律的に協調する。
本番（Phase 4以降）では Lambda 15分制限を考慮し、Step Functions で4分割実行に切り替える。

```python
# swarm_local.py — ローカル開発・デバッグ用

from strands import Agent
from strands.multiagent import Swarm
from strands.types.content import ContentBlock
from tools import (
    fetch_article_content, fetch_article_images,
    get_past_threads, get_same_article_threads, save_post,
)

# --- エージェント定義 ---
claude_engineer = Agent(
    name="claude_engineer",
    description="辛口エンジニア。GitHubのコードとプロンプトを読み、技術的ツッコミを行う",
    system_prompt=CLAUDE_SYSTEM_PROMPT,  # §7 参照
    tools=[
        fetch_article_content, fetch_article_images,
        get_past_threads, get_same_article_threads, save_post,
        # GitHub MCP tools（Gateway経由）
    ],
)

gpt_tax_advisor = Agent(
    name="gpt_tax_advisor",
    description="独立系税理士。手順の再現性とビジネス実用度を評価する",
    system_prompt=GPT_SYSTEM_PROMPT,
    tools=[
        fetch_article_content, fetch_article_images,
        get_past_threads, get_same_article_threads, save_post,
        # GitHub MCP tools（README系のみ）
    ],
)

gemini_mother = Agent(
    name="gemini_mother",
    description="子育てお母さん。人間味と共感の視点でレビューする",
...
swarm_moderator = Agent(
    name="swarm_moderator",
    description="3者の議論を踏まえ、総合評価点を決定するモデレーター",
    system_prompt=MODERATOR_SYSTEM_PROMPT,
    tools=[save_post],  # 総合評価の保存のみ
)

# --- Swarm 構成 ---
swarm = Swarm(
    [claude_engineer, gpt_tax_advisor, gemini_mother, swarm_moderator],
    entry_point=claude_engineer,
    max_handoffs=8,            # 4エージェント × 2（余裕）
    max_iterations=12,
    execution_timeout=900.0,   # 15分
    node_timeout=300.0,        # 5分/エージェント
    repetitive_handoff_detection_window=6,
    repetitive_handoff_min_unique_agents=3,
)

# --- 実行 ---
# マルチモーダル入力（記事テキスト + スクリーンショット画像）
content_blocks = [
    ContentBlock(text=f"以下の記事をレビューしてください:\n{article_text}"),
    ContentBlock(image={"format": "png", "source": {"bytes": screenshot_bytes}}),
]
...
### 本番構成（Step Functions 4分割）

本番では各エージェントを個別 Lambda で実行し、Step Functions が順次呼び出す（§10参照）。
前のエージェントの書き込みは DynamoDB から取得、または Step Functions の state passing で渡す。
Swarm の shared context 相当は、各 Lambda に同じ `article_context` を渡すことで実現する。
...
## 10. Step Functions 定義（本番用・4分割）

ローカル開発では Strands `Swarm` クラスを使うが（§6参照）、本番では Lambda 15分制限を考慮し、
エージェントごとに個別 Lambda として Step Functions で順次実行する。

```json
{
  "Comment": "okamoちゃんねる Daily Thread Pipeline（本番: 4分割Lambda）",
  ...
  "States": {
    ...
    "GPTReview": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:...:review-agent",
      "Comment": "独立系税理士。Claudeの書き込みを踏まえてレス",
      "Parameters": {
    ...
    "GeminiReview": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:...:review-agent",
      "Comment": "子育てお母さん。Claude+GPTの書き込みを踏まえてレス",
      "Parameters": {
      ...
      "Next": "ModeratorSummary",
      "Retry": [{"ErrorEquals": ["States.TaskFailed"], "MaxAttempts": 1}]
    },
    "ModeratorSummary": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:...:review-agent",
      "Comment": "モデレーター。3者の議論を踏まえ総合評価を決定",
      "Parameters": {
        "agent_id": "swarm_moderator",
        "post_number": "005",
        "article_context.$": "$.article_context",
        "all_posts": {
          "claude.$": "$.claude_result.post",
    ...
    "PublishThread": {
      ...
      "Comment": "静的HTML + KB用MD生成・S3公開",
      "Parameters": {
        "thread_date.$": "$.article_context.date",
        "article.$": "$.article_context.article"
      },
      "Comment2": "全レスはDynamoDBから取得。state passingは不要",
      "End": true
    }
  }
}
