...
| `get_past_threads` | @tool | DynamoDBから直近3回分のスレッド全文取得（okamoの書き込み含む） |
...
- **L1のみ（直近3回分のスレッド全文）** から開始する
- L2（週次ダイジェスト）/ L3（RAG検索）は後から追加判断

### L1で始める理由

1. **ベースライン確立**: L1のみの出力品質がわかって初めて、L2/L3の価値が測れる
2. **コスト感覚**: 3エージェント × 3回分全文のトークン消費量を実数で把握
3. **YAGNI**: L1で十分かもしれない。不十分と判明してからL2/L3を足す

### L1に含まれるもの

- 直近3回分のスレッド全文（AI書き込み＋okamoの手動書き込み）— `get_past_threads`
...
@tool
def get_past_threads(count: int = 3) -> str:
    """直近N回分のスレッド全文をDynamoDBから取得する。
    日付ではなく実行回数ベース（週1起動でも直近3回分を取得）。
    okamoの手動書き込みも含む。"""
...
| **Phase 5** | DynamoDB保存 + L1（直近3回分）注入 | 議論がコンテキストを踏まえて深化するか |
